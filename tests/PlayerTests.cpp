 ...
 *
 * The real StudioApplication, not a stand-in: play mode is exactly the feature where a test double
...
namespace
{
    struct PlayFixture
    {
        ...
        {
            const auto& ui = static_cast<const NullStudioUi&>(
...
CNA_STUDIO_TEST(PlayRefusesWithNoProjectAndSaysWhy)
{
    PlayFixture fixture;
    ...
    CNA_STUDIO_EXPECT(fixture.application.getPlayMode() == PlayMode::Stopped);
    ...
    CNA_STUDIO_EXPECT(fixture.logContains("open project"));
}

CNA_STUDIO_TEST(PlayRefusesWhenNoPlayerBuildIsInstalled)
{
    ...
    CNA_STUDIO_EXPECT(fixture.application.getPlayMode() == PlayMode::Stopped);
    CNA_STUDIO_EXPECT(fixture.logContains("No cna-player build"));
}

CNA_STUDIO_TEST(PlayControlsAreInertWhileStopped)
{
...
CNA_STUDIO_TEST(TheStudioDrivesARealPlayerThroughPlayPauseStepAndStop)
{
    ...
    fixture.application.startPlay();
    CNA_STUDIO_EXPECT(fixture.application.getPlayMode() == PlayMode::Playing);
    if (fixture.application.getPlayMode() != PlayMode::Playing)
    {
        ::CnaStudioTest::reportFailure(__FILE__, __LINE__, "player did not start");
        return;
    }

    CNA_STUDIO_EXPECT(fixture.pumpUntil([&] { return fixture.logContains("Player ready"); }));

    fixture.application.setPlayPaused(true);
    CNA_STUDIO_EXPECT(fixture.application.getPlayMode() == PlayMode::Paused);
    ...
    CNA_STUDIO_EXPECT(fixture.pumpUntil([&] { return fixture.logContains("player: paused"); }));

    fixture.application.stepPlayFrame();

    fixture.application.setPlayPaused(false);
    CNA_STUDIO_EXPECT(fixture.application.getPlayMode() == PlayMode::Playing);
    CNA_STUDIO_EXPECT(fixture.pumpUntil([&] { return fixture.logContains("player: resumed"); }));
...
namespace
{
    ...
    class InputScriptedUi final : public NullStudioUi
    {
    public:
...
CNA_STUDIO_TEST(LiveEditsAndAssetChangesReachARunningPlayer)
{
    ...
    CNA_STUDIO_EXPECT(application.getPlayMode() == PlayMode::Playing);
    ...
    CNA_STUDIO_EXPECT(sawLog("Player ready"));
    ...
    CNA_STUDIO_EXPECT(sawLog("set Player.CNA.Transform.position"));
    ...
    CNA_STUDIO_EXPECT(sawLog("reloaded 'Assets/hero.png'"));
    ...
    CNA_STUDIO_EXPECT(seen.isKeyDown("W"));
    CNA_STUDIO_EXPECT(seen.leftButton);
    ...
    CNA_STUDIO_EXPECT(seen.hasPointer());
    CNA_STUDIO_EXPECT(std::abs(seen.mouseX / seen.surfaceWidth - 0.25f) < 0.01f);

    application.stopPlay();
    CNA_STUDIO_EXPECT(application.getPlayMode() == PlayMode::Stopped);
}
