...
creation ──▶ the .cnaproject, the directory layout, and refusing before anything is written
```

That split is what §13 and `STUDIO-08005` are both for. A template carrying a `CMakeLists.txt`
would be a template secretly about C++, and there would be one copy of it per template to keep in
step; a language carrying a starting scene would be a language secretly about 3D.

**A template is data plus a file tree.** A directory holding a `template.json` and a `content/`
subtree, found on a search path — beside the executable for an installed Studio, and the source
tree baked in at configure time for a development build. Nothing about a template is compiled,
registered, or listed in a C++ table, and `STUDIO-08011`'s CTest cases are generated by globbing
the same directory, so a template added without touching code is a template CI creates and builds.

**A project Studio creates is buildable in place, not only after being exported.** `Scenes/` and
`Assets/` stay where the editor writes them and the generated build stages them beside the
executable, so the same directory is the authoring tree and the source tree — which is how every
other C++ project on the machine works. Export remains what it always was: a copy of the project
somewhere else, for shipping.

**Creation refuses before it writes, never during.** A creation that failed halfway would leave a
directory that is neither a project nor empty, and the user would be left working out which files
were theirs. Validation is a separate function with its own tests, creation calls it first and
stops, and the `.cnaproject` is written *last* — a directory with one in it is one the Hub lists as
a project.

**Content is copied byte for byte, UUIDs included.** Two projects created from one template share
their scene and entity ids, which is harmless — those ids are scoped to a scene — and it is what
makes creation reproducible. Generating fresh ids would make the same template produce a different
tree every time, which is a worse property than the one it fixes.

**Opening never refuses over what is wrong with a project.** A project whose startup scene is
missing, whose renderer no longer exists or whose language this build cannot author still opens,
