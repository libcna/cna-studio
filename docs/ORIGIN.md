...
CNA Studio began as a one-time source import of the **CNA Editor**, which was developed
...
The **contents** of `cna-editor/` became the **root** of this repository. There is no
`cna-studio/cna-editor/` directory: `cna-editor/CMakeLists.txt` became `./CMakeLists.txt`,
`cna-editor/src/` became `./src/`, and so on.
...
`cna-lab/cna-editor` is the historical source snapshot the product was bootstrapped from. It
is **read-only** material for this project: useful for comparing against the original
implementation, and nothing else.

There is **no automatic synchronisation** between `cna-lab/cna-editor` and this repository,
