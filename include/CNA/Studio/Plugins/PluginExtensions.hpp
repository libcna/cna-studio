...
/**
 * @file CNA/Studio/Plugins/PluginExtensions.hpp
 ...
 *   `StudioContext` and `ComponentRegistry::registerComponent` is already the supported way in --
...
namespace CNA::Studio
{
    ...
    struct PluginPanel
    {
         ...
         *
         * Handed the `StudioUi` and the context, so a plugin panel has exactly what a built-in one
         ...
         */
        std::function<void(StudioUi&, StudioContext&)> draw;
    };
    ...
    struct PluginMenuCommand
    {
        ...
        std::function<void(StudioContext&)> invoke;
    };

    /**
     * @brief Panels and menu commands contributed by plugins (ED-412).
     *
     * Held by `StudioContext`, which is what a plugin is handed, so registering is one call with
