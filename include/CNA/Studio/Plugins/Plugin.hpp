...
/**
 * @file CNA/Studio/Plugins/Plugin.hpp
...
namespace CNA::Studio
{
    ...
    inline constexpr int kStudioPluginApiVersion = 1;
    ...
    struct PluginManifest
    {
        ...
        /** @brief Returns true when @p studioApiVersion matches this build exactly. */
        [[nodiscard]] bool isCompatible() const { return studioApiVersion == kStudioPluginApiVersion; }
    };
    ...
    public:
         ...
         */
        virtual void initialize(StudioContext& context) = 0;

        /** @brief Called once before unloading. Must remove everything initialize() registered. */
        virtual void shutdown(StudioContext& context) = 0;
    ...
    /**
     * @brief The signature the plugin library must export as `cnaStudioCreatePlugin`.
     *
     * The returned object is owned by the caller and destroyed through
     * `cnaStudioDestroyPlugin`, so that allocation and deallocation both happen inside the
     ...
     */
    using CreatePluginFunction = StudioPlugin* (*)(int studioApiVersion);

    /** @brief The signature the plugin library must export as `cnaStudioDestroyPlugin`. */
    using DestroyPluginFunction = void (*)(StudioPlugin*);
    ...
    public:
         ...
         */
        std::size_t loadAll(StudioContext& context);
         ...
         */
        void unloadAll(StudioContext& context);
         ...
         */
        bool reload(StudioContext& context, const std::string& pluginId);
    ...
    private:
        ...
        bool activate(StudioContext& context, std::size_t index);

        /** @brief Shuts down, destroys and closes @p entry. Never throws. */
        void deactivate(StudioContext& context, Open& entry);
    };
}
