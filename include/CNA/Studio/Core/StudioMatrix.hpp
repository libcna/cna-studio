...
/**
 * @file CNA/Studio/Core/StudioMatrix.hpp
 ...
 * `StudioMath.hpp` does (ANALYSIS.md D-03): cna-studio-core links nothing from CNA, so a camera
 * built on this is unit-testable with no window, no GPU and no CNA checkout, and the conversion in
 * cna-studio-viewport stays a field copy.
 *
...
namespace CNA::Studio
{
     ...
     *
     * XNA's own `Matrix()` is zero-filled; this one is the identity, following `StudioQuaternion`,
    ...
    struct StudioMatrix
    {
        ...
        friend bool operator==(const StudioMatrix& lhs, const StudioMatrix& rhs);
        friend bool operator!=(const StudioMatrix& lhs, const StudioMatrix& rhs) { return !(lhs == rhs); }
    };

    /** @brief Returns the transform @p a followed by the transform @p b, i.e. the product `a * b`. */
    [[nodiscard]] StudioMatrix multiply(const StudioMatrix& a, const StudioMatrix& b);
     ...
     */
    [[nodiscard]] std::optional<StudioMatrix> invert(const StudioMatrix& matrix);
     ...
     */
    [[nodiscard]] StudioMatrix transpose(const StudioMatrix& matrix);
     ...
     */
    [[nodiscard]] std::optional<StudioMatrix> inverseTranspose(const StudioMatrix& matrix);

    /** @brief Returns @p point transformed by @p matrix, treating it as a position (w = 1). */
    [[nodiscard]] StudioVector3 transformPosition(const StudioMatrix& matrix, const StudioVector3& point);
     ...
     */
    [[nodiscard]] StudioVector3 transformDirection(const StudioMatrix& matrix, const StudioVector3& direction);
     ...
     */
    [[nodiscard]] StudioVector3 transformWithPerspective(const StudioMatrix& matrix,
                                                         const StudioVector3& point, float& outW);

    /** @brief Returns a translation by @p offset. */
    [[nodiscard]] StudioMatrix createTranslation(const StudioVector3& offset);

    /** @brief Returns a scale by @p factors, per axis. */
    [[nodiscard]] StudioMatrix createScale(const StudioVector3& factors);

    /** @brief Returns the rotation @p rotation as a matrix. */
    [[nodiscard]] StudioMatrix createFromQuaternion(const StudioQuaternion& rotation);
     ...
     */
    [[nodiscard]] StudioMatrix createLookAt(const StudioVector3& eye, const StudioVector3& target,
                                            const StudioVector3& up);
     ...
     */
    [[nodiscard]] StudioMatrix createPerspectiveFieldOfView(float fieldOfViewRadians, float aspectRatio,
                                                            float nearPlane, float farPlane);

    /** @brief Returns a right-handed orthographic projection of the given world-space extent. */
    [[nodiscard]] StudioMatrix createOrthographic(float width, float height, float nearPlane, float farPlane);
}
