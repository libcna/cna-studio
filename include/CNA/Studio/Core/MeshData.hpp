...
/**
 * @file CNA/Studio/Core/MeshData.hpp
 * @brief The seam that carries mesh geometry from an importer to whatever draws it (plan.md ED-405).
 *
 * This file is the *interface* between the glTF importer in `cna-studio-assets` and the two things
 * that will draw what it produces: the CNA-free wireframe in `cna-studio-scene`, and -- when
 * ED-402 arrives -- the `VertexBuffer`/`BasicEffect` pass in `cna-studio-viewport`. plan.md's
 ...
 *
 * **1. It lives in `cna-studio-core`, which is not where either party lives.** The producer is
 * `cna-studio-assets` and one consumer is `cna-studio-scene`, and those two modules link nothing
 * but core -- neither can see the other, and neither should. Core is the only place a type can sit
 * where both can reach it, which is the same reason `StudioMatrix` is in core while the camera
...
namespace CNA::Studio
{
    ...
    struct MeshVertex
    {
         ...
         */
        StudioVector3 normal{0.0f, 0.0f, 1.0f};

        StudioVector2 texCoord;
    };
    ...
    struct MeshMaterial
    {
         ...
         */
        StudioVector3 specularColor{0.0f, 0.0f, 0.0f};
    ...
    struct MeshData
    {
         ...
         *
         * Two loose `StudioVector3` rather than the `WorldBounds3D` the 3D camera uses, because
         * that type lives in `cna-studio-scene` and this file may not depend on it -- scene is a
         ...
         */
        StudioVector3 boundsMin;
        StudioVector3 boundsMax;
     ...
     * (`cna-studio-assets`, which caches these) and the consumers (`cna-studio-scene`'s wireframe,
     * and `cna-studio-viewport` when ED-402 lands) cannot see each other and must not need to.
     *
