...
/**
 * @file CNA/Studio/Core/StudioCommand.hpp
 ...
 * a plugin, from the runtime bridge -- is an StudioCommand pushed through CommandHistory. That is
...
namespace CNA::Studio
{
    ...
    public:
         ...
         *         as a separate entry.
         */
        virtual bool mergeWith(const StudioCommand& newer) { (void)newer; return false; }
    };
    ...
    public:
        ...
        {
            for (const std::unique_ptr<StudioCommand>& command : commands_) { command->execute(); }
        }
    ...
    public:
         ...
         */
        [[nodiscard]] const StudioCommand* getCommandAt(std::size_t index) const;
