...
/**
 * @file CNA/Studio/RuntimeBridge/StudioProtocol.hpp
...
{
    /** @brief Message kinds. Studio-to-player and player-to-editor share one enumeration. */
    enum class StudioMessageType
    {
        Unknown,
    ...
    {
        StudioMessageType type = StudioMessageType::Unknown;
         ...
         */
        static std::optional<StudioMessage> decode(std::string_view line);

        /** @brief Builds a Hello message. */
        static StudioMessage makeHello(const std::string& projectRoot);

        /** @brief Builds a LoadScene message for a project-relative @p scenePath. */
        static StudioMessage makeLoadScene(const std::string& scenePath);
         ...
         */
        static StudioMessage makeReloadAsset(const Uuid& assetId);

        /** @brief Builds a SetProperty message targeting one property of one live entity. */
        static StudioMessage makeSetProperty(const Uuid& entityId,
         ...
         */
        static StudioMessage makeScreenshot(const std::string& path);

        /** @brief Builds an Input message carrying @p snapshot. */
        static StudioMessage makeInput(const PlayerInputSnapshot& snapshot);

        /** @brief Builds the player's ReportInput reply, carrying the snapshot it now holds. */
        static StudioMessage makeReportInput(const PlayerInputSnapshot& snapshot);

        /** @brief Builds a ReportLog message. */
        static StudioMessage makeReportLog(const std::string& severity, const std::string& text);
    };

    /** @brief The protocol version this build speaks; sent in Hello and checked in Ready. */
    inline constexpr int kStudioProtocolVersion = 1;
