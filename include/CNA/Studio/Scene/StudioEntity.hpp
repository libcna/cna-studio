...
/**
 * @file CNA/Studio/Scene/StudioEntity.hpp
...
namespace CNA::Studio
{
    ...
    public:
        ...
        explicit StudioComponent(std::string typeId) : typeId_(std::move(typeId)) {}
    ...
    {
    public:
        StudioEntity() = default;
        StudioEntity(Uuid id, std::string name) : id_(id), name_(std::move(name)) {}
        ...
        [[nodiscard]] const std::vector<StudioComponent>& getComponents() const { return components_; }
        [[nodiscard]] std::vector<StudioComponent>& getComponents() { return components_; }

        /** @brief Returns the first component of @p typeId, or nullptr when there is none. */
        [[nodiscard]] const StudioComponent* findComponent(std::string_view typeId) const;
        [[nodiscard]] StudioComponent* findComponent(std::string_view typeId);

        /** @brief Appends @p component and returns a reference to the stored copy. */
        StudioComponent& addComponent(StudioComponent component);
        ...
        /**
         * @brief Studio-only, non-runtime state.
         *
         ...
         */
        [[nodiscard]] const std::map<std::string, PropertyValue>& getStudioState() const { return studioState_; }
        void setStudioState(std::string name, PropertyValue value);
         ...
         */
        bool removeStudioState(std::string_view name);
    ...
    private:
        ...
        bool enabled_ = true;
        int sortOrder_ = 0;
        std::vector<StudioComponent> components_;
        std::map<std::string, PropertyValue> studioState_;
    };
}
