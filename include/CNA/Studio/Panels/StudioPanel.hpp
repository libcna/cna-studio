...
/**
 * @file CNA/Studio/Panels/StudioPanel.hpp
 * @brief The base a panel derives from, and the editor operations it may invoke.
 *
 * Panels were methods on StudioApplication through Phase 0, which was right while there was barely
 ...
 *
 * What a panel may *do* to the editor goes through StudioActions rather than through a back
...
namespace CNA::Studio
{
    ...
    public:
         ...
         * nothing.
         */
        [[nodiscard]] virtual StudioAudio& getAudio() = 0;
        ...
        virtual void setStudioTool(StudioTool tool) = 0;
        [[nodiscard]] virtual StudioTool getStudioTool() const = 0;
         ...
         * other. Studio state throughout: none of it reaches the document (D-07).
         */
    ...
    {
    public:
        StudioPanel(StudioContext& context, StudioUi& ui, StudioActions& actions)
            : context_(context), ui_(ui), actions_(actions)
        {
        }

        virtual ~StudioPanel() = default;

        StudioPanel(const StudioPanel&) = delete;
        StudioPanel& operator=(const StudioPanel&) = delete;
