...
/**
 * @file CNA/Studio/Panels/ViewportPanel.hpp
...
namespace CNA::Studio
{
    ...
    class ViewportPanel final : public StudioPanel
    {
    public:
    ...
    private:
         ...
         */
        void handleInteraction3D(const UiImageInteraction& interaction, const StudioVector2& cursor);
        ...
        void updateMulti3DDrag(const StudioVector2& cursor, const GizmoSnap& snap);
         ...
         */
        [[nodiscard]] std::optional<StudioVector3> getGizmo3DPivot() const;
         ...
         */
        bool beginGizmo3DDrag(const UiImageInteraction& interaction, const StudioVector2& cursor);
        ...
        void updateGizmo3DDrag(const StudioVector2& cursor, const GizmoSnap& snap);
         ...
         */
        bool updateActiveDrag(const UiImageInteraction& interaction, const StudioVector2& cursor);

        /** @brief Applies this frame's input to the active tool. */
        void applyToolInput(const UiImageInteraction& interaction, const StudioVector2& cursor);

        /** @brief Applies wheel zoom and drag panning, which every tool leaves alone. */
        void updateCamera(const UiImageInteraction& interaction, const StudioVector2& cursor);
         ...
         * @return True when a drag began, in which case the press must not also reach the picker.
         */
        bool beginGizmoDrag(const StudioVector2& cursor);
        ...
        void updateGizmoDrag(const StudioVector2& cursor, const GizmoSnap& snap);
        ...
        void pickTileAt(const StudioVector2& cursor);

        /** @brief Fills the rectangle between the press cell and @p cursor, as one undo entry. */
        void fillTilesTo(const StudioVector2& cursor);
         ...
         */
        [[nodiscard]] std::optional<TileCoordinate> tileUnder(const StudioVector2& cursor,
         ...
         */
        void paintTileAt(const StudioVector2& cursor, bool startStroke);
         ...
         */
        [[nodiscard]] std::optional<StudioVector2> getGizmoPivot() const;

        /** @brief Applies an in-progress multi-selection drag as one command. */
        void updateMultiDrag(const StudioVector2& cursor, const GizmoSnap& snap);
