...
namespace CNA::Studio
{
        ...
        void captureDrawData()
        {
            ...
            for (int listIndex = 0; listIndex < source->CmdListsCount; ++listIndex)
            {
                ...
                static_assert(sizeof(ImDrawIdx) == sizeof(std::uint16_t),
                              "cna-studio assumes ImGui's 16-bit index type; see imconfig.h");
    ...
    ImGuiStudioUi::ImGuiStudioUi() : impl_(std::make_unique<Impl>())
    {
        IMGUI_CHECKVERSION();
        ...
        io.BackendPlatformName = "cna-studio";
        io.BackendRendererName = "cna-studio-viewport";
    ...
    bool ImGuiStudioUi::isRunning() const { return impl_->running; }

    void ImGuiStudioUi::setInput(const UiInputState& input) { impl_->input = input; }

    const UiDrawData& ImGuiStudioUi::getDrawData() const { return impl_->drawData; }

    void ImGuiStudioUi::setClipboardHooks(const UiClipboardHooks& hooks) { clipboardHooks() = hooks; }

    bool ImGuiStudioUi::isAnyItemActive() const { return ImGui::IsAnyItemActive(); }

    void ImGuiStudioUi::requestExit() { impl_->running = false; }

    bool ImGuiStudioUi::wantsTextInput() const
    {
        ImGui::SetCurrentContext(impl_->context);
        return ImGui::GetIO().WantTextInput;
    }

    bool ImGuiStudioUi::wantsMouseCapture() const
    {
        ImGui::SetCurrentContext(impl_->context);
    ...
    void ImGuiStudioUi::beginDockSpace()
    {
        const ImGuiViewport* viewport = ImGui::GetMainViewport();
        ...
        ImGui::Begin("##CnaStudioDockHost", nullptr, flags);
        ImGui::PopStyleVar(3);

        const ImGuiID dockspaceId = ImGui::GetID("CnaStudioDockSpace");
    ...
    void ImGuiStudioUi::endDockSpace()
    {
        ImGui::EndMenuBar();
        ...
        if (impl_->buildingLayout)
        {
            ...
            ImGui::DockBuilderFinish(ImGui::GetID("CnaStudioDockSpace"));
    ...
    void ImGuiStudioUi::requestPanelFocus(const std::string& title)
    {
        impl_->focusRequest = title;
    }

    bool ImGuiStudioUi::beginPanel(const std::string& title, DockSide preferredSide)
    {
    ...
    void ImGuiStudioUi::text(const std::string& value) { ImGui::TextUnformatted(value.c_str()); }

    bool ImGuiStudioUi::button(const std::string& label) { return ImGui::Button(label.c_str()); }

    bool ImGuiStudioUi::checkbox(const std::string& label, bool& value)
    {
        return ImGui::Checkbox(label.c_str(), &value);
    }

    bool ImGuiStudioUi::propertyField(const std::string& label,
                                      PropertyValue& value,
                                      ...
                                      bool readOnly)
    {
        ...
        switch (value.getType())
        {
            ...
            case PropertyType::Color: {
                const StudioColor held = value.get<StudioColor>();
                ...
                {
                    value = PropertyValue{StudioColor{
            ...
            case PropertyType::Quaternion: {
                ...
                {
                    const StudioVector4 held = value.get<StudioVector4>();
                    components[0] = held.x; components[1] = held.y;
                    components[2] = held.z; components[3] = held.w;
                }
                else
                {
                    const StudioQuaternion held = value.get<StudioQuaternion>();
                    components[0] = held.x; components[1] = held.y;
                    components[2] = held.z; components[3] = held.w;
                }
                if (ImGui::DragFloat4(label.c_str(), components, 0.01f, 0.0f, 0.0f, "%.3f", flags) && !readOnly)
                {
                    if (value.getType() == PropertyType::Vector4)
                    {
                        value = PropertyValue{StudioVector4{components[0], components[1],
                                                            components[2], components[3]}};
                    }
                    else
                    {
                        value = PropertyValue{StudioQuaternion{components[0], components[1],
                                                                components[2], components[3]}};
                    }
                    changed = true;
                }
            ...
            case PropertyType::Rectangle: {
                const StudioRectangle held = value.get<StudioRectangle>();
                int components[4] = {held.x, held.y, held.width, held.height};
                if (ImGui::InputInt4(label.c_str(), components, flags) && !readOnly)
                {
                    value = PropertyValue{StudioRectangle{components[0], components[1],
                                                           components[2], components[3]}};
                    changed = true;
                }
                break;
    ...
    UiTreeNodeResult ImGuiStudioUi::treeNode(const std::string& id,
    ...
    UiTreeNodeResult ImGuiStudioUi::treeNode(const Uuid& id,
                                             const std::string& label,
                                             bool selected,
                                             bool leaf)
    {
    ...
    void ImGuiStudioUi::treePop() { ImGui::TreePop(); }

    UiTextFieldResult ImGuiStudioUi::inputText(const std::string& id, std::string& text, bool takeFocus)
    {
    ...
    UiKeyModifiers ImGuiStudioUi::getModifiers() const
    {
    ...
    void ImGuiStudioUi::setDragSource(const std::string& type,
    ...
    std::optional<std::string> ImGuiStudioUi::acceptDrop(const std::string& type)
    {
    ...
    bool ImGuiStudioUi::beginContextMenu(const std::string& id)
    {
        return ImGui::BeginPopupContextItem(id.c_str());
    }

    void ImGuiStudioUi::endContextMenu() { ImGui::EndPopup(); }

    bool ImGuiStudioUi::beginMenu(const std::string& label) { return ImGui::BeginMenu(label.c_str()); }

    void ImGuiStudioUi::endMenu() { ImGui::EndMenu(); }

    bool ImGuiStudioUi::menuItem(const std::string& label, const std::string& shortcut, bool enabled)
    {
        return ImGui::MenuItem(label.c_str(), shortcut.empty() ? nullptr : shortcut.c_str(), false, enabled);
    }

    void ImGuiStudioUi::drawLogView(const UiLogViewOptions& options)
    {
    ...
    UiRegion ImGuiStudioUi::getContentRegion() const
    {
    ...
    void ImGuiStudioUi::imageRegion(const std::string& id,
                                    UiTextureId texture,
                                    const StudioRectangle& source,
                                    const StudioVector2& sourceSize,
                                    float width,
                                    float height)
    {
    ...
    UiImageInteraction ImGuiStudioUi::image(const std::string& id,
                                            UiTextureId texture,
                                            float width,
                                            float height,
                                            bool flipVertically)
    ...
    bool ImGuiStudioUi::isKeyDown(UiKey key) const
    {
    ...
    bool ImGuiStudioUi::isShortcutPressed(UiKey key, UiKeyModifiers modifiers)
    {
        const ImGuiKey imguiKey = toImGuiKey(key);
        if (imguiKey == ImGuiKey_None) { return false; }
    ...
    void ImGuiStudioUi::setNextItemWidth(float width) { ImGui::SetNextItemWidth(width); }

    void ImGuiStudioUi::separator() { ImGui::Separator(); }

    void ImGuiStudioUi::sameLine() { ImGui::SameLine(); }
    ...
    void ImGuiStudioUi::saveLayout() const
    {
