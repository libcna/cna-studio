...
namespace CNA::Studio
{
    ...
    void WorldBounds3D::encapsulate(const StudioVector3& point)
    {
    ...
    WorldBounds3D WorldBounds3D::makeEmpty()
    {
        ...
        return WorldBounds3D{StudioVector3{infinity, infinity, infinity},
                             StudioVector3{-infinity, -infinity, -infinity}};
    }

    void StudioCamera3D::setDistance(float distance)
    {
        distance_ = clampTo(distance, kMinDistance, kMaxDistance);
    }

    void StudioCamera3D::setYaw(float radians)
    {
    ...
    void StudioCamera3D::setPitch(float radians)
    {
        pitch_ = clampTo(radians, -kMaxPitchRadians, kMaxPitchRadians);
    }

    void StudioCamera3D::setFieldOfView(float radians)
    {
    ...
    void StudioCamera3D::setClipPlanes(float nearPlane, float farPlane)
    {
    ...
    StudioVector3 StudioCamera3D::getForward() const
    {
        ...
        return StudioVector3{-std::sin(yaw_) * cosPitch, std::sin(pitch_), -std::cos(yaw_) * cosPitch};
    }
    ...
    StudioVector3 StudioCamera3D::getEye() const
    {
        return subtract(pivot_, scale(getForward(), distance_));
    }

    float StudioCamera3D::getOrthographicHeight() const
    {
    ...
    StudioMatrix StudioCamera3D::getProjectionMatrix() const
    {
        const float aspect =
    ...
    StudioMatrix StudioCamera3D::getViewProjectionMatrix() const
    {
        ...
        // what is drawn.
        static const StudioMatrix kFlipY = createScale(StudioVector3{1.0f, -1.0f, 1.0f});

        return multiply(getViewMatrix(), multiply(getProjectionMatrix(), kFlipY));
    }

    std::optional<StudioVector2> StudioCamera3D::worldToScreen(const StudioVector3& world) const
    {
        float w = 0.0f;
        const StudioVector3 clip = transformWithPerspective(getViewProjectionMatrix(), world, w);
        if (w <= 0.0f) { return std::nullopt; }

        // Clip space is [-1, 1] across and up; the viewport's origin is its top-left, so Y flips.
        return StudioVector2{(clip.x * 0.5f + 0.5f) * viewportSize_.x,
                             (0.5f - clip.y * 0.5f) * viewportSize_.y};
    }

    WorldRay StudioCamera3D::screenToRay(const StudioVector2& screen) const
    {
        WorldRay ray;
        ray.origin = getEye();
        ray.direction = getForward();

        const std::optional<StudioMatrix> inverse = invert(getViewProjectionMatrix());
        ...
        float nearW = 0.0f;
        float farW = 0.0f;
        const StudioVector3 nearPoint =
            transformWithPerspective(*inverse, StudioVector3{clipX, clipY, 0.0f}, nearW);
        const StudioVector3 farPoint =
            transformWithPerspective(*inverse, StudioVector3{clipX, clipY, 1.0f}, farW);

        const StudioVector3 direction = normalize(subtract(farPoint, nearPoint));
    ...
    void StudioCamera3D::orbit(float yawRadians, float pitchRadians)
    {
        setYaw(yaw_ + yawRadians);
        setPitch(pitch_ + pitchRadians);
    }

    void StudioCamera3D::look(float yawRadians, float pitchRadians)
    {
        ...
        const StudioVector3 eye = getEye();
        orbit(yawRadians, pitchRadians);
        pivot_ = add(eye, scale(getForward(), distance_));
    }

    void StudioCamera3D::moveLocal(const StudioVector3& delta)
    {
        const StudioVector3 offset = add(add(scale(getRight(), delta.x), scale(getUp(), delta.y)),
                                         scale(getForward(), delta.z));
        pivot_ = add(pivot_, offset);
    }

    void StudioCamera3D::panByScreenDelta(const StudioVector2& screenDelta)
    {
        ...
        moveLocal(StudioVector3{-screenDelta.x * worldPerPixel, screenDelta.y * worldPerPixel, 0.0f});
    }

    void StudioCamera3D::dolly(float factor)
    {
        if (factor <= 0.0f) { return; }
        setDistance(distance_ * factor);
    }

    void StudioCamera3D::frame(const WorldBounds3D& bounds, float marginFraction)
    {
                                                       ...
                                                       const SpriteSizeProvider& sizeProvider)
    {
        ...
        if (flat)
        {
            return WorldBounds3D{StudioVector3{flat->min.x, flat->min.y, world->position.z},
