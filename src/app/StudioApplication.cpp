...
namespace CNA::Studio
{
    ...
    std::string StudioOptions::getUsage()
    {
        return
            ...
            "  --ui=NAME          UI toolkit: 'studio' (default on a CNA build), 'imgui' (the\n"
            "                     legacy editor, kept as a migration fallback) or 'null'.\n"
    ...
    StudioApplication::StudioApplication(std::unique_ptr<StudioUi> ui, std::unique_ptr<StudioViewport> viewport)
        : ui_(std::move(ui)),
          ...
          comparisonPanel_(context_, *ui_, *this)
    {
        ...
        context_.setCommandObserver([this](const StudioCommand& command) { mirrorToPlayer(command); });
    }

    void StudioApplication::setViewport(std::unique_ptr<StudioViewport> viewport)
    {
        if (!viewport) { return; }
        ...
        const StudioCamera2D previousCamera = viewport_ ? viewport_->getCamera() : StudioCamera2D{};
        const StudioCamera3D previousCamera3D = viewport_ ? viewport_->getCamera3D() : StudioCamera3D{};
        viewport_ = std::move(viewport);
    ...
    bool StudioApplication::initialize(const StudioOptions& options)
    {
    ...
    void StudioApplication::updateBackendComparison()
    {
    ...
    void StudioApplication::setPlayerBuilds(std::vector<PlayerBuild> builds)
    {
    ...
    void StudioApplication::renderFrame(double deltaSeconds)
    {
    ...
    void StudioApplication::handleShortcuts()
    {
    ...
    void StudioApplication::setStudioTool(StudioTool tool)
    {
    ...
    void StudioApplication::newScene() { context_.newScene(); }

    void StudioApplication::saveScene()
    {
    ...
    void StudioApplication::undo()
    {
        ...
        if (const StudioCommand* entry = history.getCommandAt(history.getCursor()))
        {
            mirrorToPlayer(*entry);
        }
    }

    void StudioApplication::redo()
    {
        CommandHistory& history = context_.getHistory();
        ...
        if (const StudioCommand* entry = history.getCommandAt(index)) { mirrorToPlayer(*entry); }
    }

    void StudioApplication::forwardInputToPlayer(const PlayerInputSnapshot& snapshot)
    {
        ...
        player_->send(StudioMessage::makeInput(snapshot));
    }

    void StudioApplication::mirrorToPlayer(const StudioCommand& command)
    {
        if (!player_ || playMode_ == PlayMode::Stopped) { return; }
        ...
        player_->send(StudioMessage::makeSetProperty(setProperty->getEntityId(),
                                                     setProperty->getComponentTypeId(),
                                                     setProperty->getPropertyName(),
                                                     component->getProperty(setProperty->getPropertyName())));
    }

    void StudioApplication::duplicateSelection()
    {
    ...
    void StudioApplication::deleteSelection()
    {
    ...
    void StudioApplication::frameSelection()
    {
    ...
    void StudioApplication::setGizmoMode(GizmoMode mode)
    {
        gizmoMode_ = mode;
    }

    void StudioApplication::startBackendComparison() { comparisonPanel_.startComparison(); }

    const BackendComparison& StudioApplication::getBackendComparison() const
    {
        return comparisonPanel_.getComparison();
    }

    void StudioApplication::frameSceneInThreeDimensions()
    {
    ...
    void StudioApplication::setThreeDimensionalView(bool enabled)
    {
    ...
    void StudioApplication::setGizmoSpace(GizmoSpace space)
    {
    ...
    void StudioApplication::startPlay()
    {
    ...
    void StudioApplication::stopPlay()
    {
        if (!player_)
        {
    ...
    void StudioApplication::setPlayPaused(bool paused)
    {
        if (!player_ || playMode_ == PlayMode::Stopped) { return; }
    ...
    void StudioApplication::stepPlayFrame()
    {
    ...
    void StudioApplication::reloadAssetInPlayer(const Uuid& assetId)
    {
        ...
        player_->send(StudioMessage::makeReloadAsset(assetId));
    }

    void StudioApplication::pollPlayer()
    {
        if (!player_) { return; }
...
namespace CNA::Studio
{
    StudioApplication::~StudioApplication() { unloadPlugins(); }

    void StudioApplication::drawPluginPanels()
    {
