...
{
    bool operator==(const StudioMatrix& lhs, const StudioMatrix& rhs)
    {
    ...
    StudioMatrix transpose(const StudioMatrix& matrix)
    {
        StudioMatrix result;
    ...
    std::optional<StudioMatrix> inverseTranspose(const StudioMatrix& matrix)
    {
        const std::optional<StudioMatrix> inverted = invert(matrix);
        if (!inverted) { return std::nullopt; }
        return transpose(*inverted);
    }

    StudioMatrix multiply(const StudioMatrix& a, const StudioMatrix& b)
    {
        StudioMatrix result;
    ...
    std::optional<StudioMatrix> invert(const StudioMatrix& matrix)
    {
    ...
    StudioVector3 transformPosition(const StudioMatrix& matrix, const StudioVector3& point)
    {
    ...
    StudioVector3 transformDirection(const StudioMatrix& matrix, const StudioVector3& direction)
    {
        return StudioVector3{
    ...
    StudioVector3 transformWithPerspective(const StudioMatrix& matrix, const StudioVector3& point, float& outW)
    {
        const StudioVector3 transformed = transformPosition(matrix, point);
    ...
    StudioMatrix createTranslation(const StudioVector3& offset)
    {
        StudioMatrix result;
        result.m41 = offset.x;
        result.m42 = offset.y;
    ...
    StudioMatrix createScale(const StudioVector3& factors)
    {
        StudioMatrix result;
    ...
    StudioMatrix createFromQuaternion(const StudioQuaternion& rotation)
    {
    ...
    StudioMatrix createLookAt(const StudioVector3& eye, const StudioVector3& target, const StudioVector3& up)
    {
        ...
        StudioVector3 backward = normalize(subtract(eye, target));
        if (length(backward) <= 0.0f) { backward = StudioVector3{0.0f, 0.0f, 1.0f}; }

        StudioVector3 right = normalize(cross(up, backward));
        if (length(right) <= 0.0f)
        {
            ...
            const StudioVector3 fallback = std::abs(backward.x) < 0.9f ? StudioVector3{1.0f, 0.0f, 0.0f}
                                                                      : StudioVector3{0.0f, 1.0f, 0.0f};
            right = normalize(cross(fallback, backward));
        }
        const StudioVector3 trueUp = cross(backward, right);
    ...
    StudioMatrix createPerspectiveFieldOfView(float fieldOfViewRadians, float aspectRatio, float nearPlane,
                                              float farPlane)
    {
    ...
    StudioMatrix createOrthographic(float width, float height, float nearPlane, float farPlane)
    {
        StudioMatrix result;
